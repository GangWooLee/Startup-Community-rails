...
class Admin::UsersController < Admin::BaseController
  ...
  def calculate_panel_stats
    stats = calculate_user_type_stats
    return if stats.empty?

    # 퍼센티지 인스턴스 변수 설정 (뷰 호환성)
    @admin_percentage = stats[:admin_percentage].round
    @oauth_percentage = stats[:oauth_percentage].round
    @normal_percentage = 100 - @admin_percentage - @oauth_percentage

    # 도넛 차트 SVG 값 계산
    chart = calculate_donut_chart_values(stats)
    @normal_dash = chart[:normal_dash]
    @oauth_dash = chart[:oauth_dash]
    @admin_dash = chart[:admin_dash]
    @oauth_offset = chart[:oauth_offset]
    @admin_offset = chart[:admin_offset]
  end
end
