...
class Admin::PostsController < Admin::BaseController
  ...
  def index
    ...
    if params[:q].present?
      keyword = "%#{params[:q]}%"
      @posts = @posts.joins(:user).where("posts.title LIKE ? OR users.name LIKE ?", keyword, keyword)
    end

    # 날짜 범위 필터링
    if params[:from_date].present?
      @posts = @posts.where("posts.created_at >= ?", Date.parse(params[:from_date]).beginning_of_day)
    end
    if params[:to_date].present?
      @posts = @posts.where("posts.created_at <= ?", Date.parse(params[:to_date]).end_of_day)
    end
