...
module Ai
  ...
  # 맥락에 따라 3-5개의 구조화된 질문을 동적으로 생성
  class FollowUpGenerator < BaseAgent
    SYSTEM_PROMPT = <<~PROMPT
      당신은 스타트업 아이디어 분석을 위한 인터뷰어입니다.
      사용자가 입력한 아이디어를 바탕으로, 더 정확한 분석을 위해 필요한 추가 질문 3-5개를 생성해주세요.
      ...
      [
        {
          ...
          content: <<~MSG
            ...
            이 아이디어에서 명확하지 않은 부분을 파악하고, 분석에 필요한 추가 정보를 얻기 위한 질문 3-5개를 JSON 형식으로 생성해주세요.
          MSG
        }
      ]
    end
    ...
    def validate_and_normalize(result)
      ...
      { questions: normalized.take(5) } # 최대 5개
    end
